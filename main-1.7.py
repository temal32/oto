...
for link in links:
    ...
    try:
        ...
        if counter < 10:
            counter = str(counter)
            counter = "0" + counter
        print(f'({counter}/11) Downloaded {link.split("/")[-1]}')
        counter = int(counter)
        ...
        if counter < 10:
            ...
            counter = str(counter)
            counter = "0" + counter
        print(f'({str(counter)}/11){space}', colored(f'Error {link.split("/")[-1]}', 'red'))
...
try:
    ...
    counter += 1
    print(f'({str(counter)}/11) Downloaded {unique_link.split("/")[-1]}')
except Exception as error____:
    counter += 1
    download_error = True
    error(f"ERROR 0x2: Critical issue occured while downloading file, {error____}")
    print(f'({str(counter)}/11)', colored(f'Error {unique_link.split("/")[-1]}', 'red'))
