...
system("cls")
system("title OT-Optimizer - www.discord.gg/ysCD3xQZYE")
username = getlogin()
...
for link in links:
    ...
    try:
        ...
        print(f'({str(counter)}/9) Downloaded {link.split("/")[-1]}')
    except Exception as error_:
        download_error = True
        error(f"ERROR 0x1: Critical issue occured while downloading file, {error_}")
        print(f'({str(counter)}/9)', colored(f'Error {link.split("/")[-1]}', 'red'))
try:
    ...
    counter += 1
    print(f'({str(counter)}/9) Downloaded {unique_link.split("/")[-1]}')
except Exception as error____:
    counter += 1
    download_error = True
    error(f"ERROR 0x2: Critical issue occured while downloading file, {error____}")
    print(f'({str(counter)}/9)', colored(f'Error {unique_link.split("/")[-1]}', 'red'))
...
if choice == "1":
    ...
    question_3 = input("Would you like to improve the TCP connection?  (danilo here)(y/n)\nEnter: ")
    ...
    if path.isfile(f"{drive_letter}\\OTO-Backup\\Backup_reg.Reg"):
        system(f"start {drive_letter}\\OTO-Backup\\Backup_reg.Reg")
