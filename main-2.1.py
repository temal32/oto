# Version: 2.1
# Release date: 17.09.2022 (dd/mm/yyyy)
...
info("More information can be found here: www.discord.gg/VeuN5QZhgh")
...
def show_menu():
    ...
    menu_contact = colored(" - Temal#5222 - ItzOwo#1414 - Danilo#0001", color="cyan")
...
def check_update():
    ...
    try:
        ...
        if float(data) > float(version):
            ...
            print(colored("Update available!", "green"))
            ...
            try:
                ...
                sleep(1)
                print("Replacing files with new version...")
                sleep(2)
                debug("Deleting old file...")
                remove(argv[0])
                run(f"OT-Optimizer_{new_version}.exe")
            except Exception as request_error:
                error(f"Could not download update, {request_error}")
                if agreed == True:
                ...
                print(colored("ERROR: Some issue occured while updating OTO.", "red"))
                input_exit()
            sleep(2)
            loading_done = True
        else:
...
def delete(path):
    try:
        ...
        error(f"Could not delete file/directory, {err}")
        if agreed == True:
            embed = DiscordEmbed(title='ERROR',
                                 description=f'**Version:**\n{version}\n**User:**\n{encry_username}\n**Time:**\n{datetime.now().astimezone(timezone("Europe/Berlin")).strftime("%H:%M:%S %d-%m-%Y")}\n**Drive-Letter:**\n{drive_letter}\n**System:**\n{name}\n**WinVer:**\n{win_ver}\n**ERROR:**\n{err}',
                                 color='ff0000')
            ...
            rsp = webhook_error_reporting.execute().status_code
            debug(f"Error-Webhook sent with status code: {rsp}")
        print(Fore.LIGHTRED_EX + "ERROR  " + Style.RESET_ALL + " -", err)
...
print("Loading...")
webhook_startup_reporting = DiscordWebhook(
    url='https://discord.com/api/webhooks/1020459536013869147/6BKHDfSrippBjur7EfT0OBtvmV4nMqrbNhkiWhrhL8S_ZCDurnH1ukQ2h-fcHR3HH6fF')
webhook_error_reporting = DiscordWebhook(
    url='https://discord.com/api/webhooks/1020459536013869147/6BKHDfSrippBjur7EfT0OBtvmV4nMqrbNhkiWhrhL8S_ZCDurnH1ukQ2h-fcHR3HH6fF')
if path.exists(f"{drive_letter}\\OTO_agreed.txt"):
    debug("Agreement file exists, getting value...")
    with open(f"{drive_letter}\\OTO_agreed.txt", "r+") as file:
        agreed = file.read()
        file.close()
...
try:
    ...
    error(ser_best_err)
    print(colored(f"Error occured while getting best server, please try again later!\nError code: {ser_best_err}", "red"))
    if agreed == True:
        embed = DiscordEmbed(title='ERROR',
                             description=f'**Version:**\n{version}\n**User:**\n{encry_username}\n**Time:**\n{datetime.now().astimezone(timezone("Europe/Berlin")).strftime("%H:%M:%S %d-%m-%Y")}\n**Drive-Letter:**\n{drive_letter}\n**System:**\n{name}\n**WinVer:**\n{win_ver}\n**ERROR:**\n{ser_best_err}',
                             color='ff0000')
...
def main_menu():
    ...
    if choice == "1":
        ...
        if yeono == True and best_server_txt == True:
            ...
            mc_new_pings = []
            abs_new_pings = []
            if path.exists(f"{drive_letter}\\OTO-Pings") == True:
                try:
                    ...
                    try:
                        for mc_ping in mc_hosts:
                            try:
                                with open(f"{drive_letter}\\OTO-Pings\\ping_{mc_ping}.txt", 'r+') as file:
                        ...
                        print("\nping comparison\n".upper())
                        for new_mc_ping, mc_hosts_names_x, abs_ping in zip(mc_hosts_names, mc_new_pings, abs_new_pings):
            ...
            if old < new:
                if new == 2000.0:
                    loading_done = True
                    clear_last_line()
                    print(colored("An issue occured while getting new ping, please try again.", "red"))
                    error("An issue occured while getting new ping, please try again. (ping == 2000)")
                else:
                    loading_done = True
        ...
        if path.isfile(f"{drive_letter}\\OTO-Backup\\Backup_reg.reg"):
            chdir(f"{drive_letter}\\OTO-Backup")
        ...
        try:
            ...
            system("color 4")
            error(f"ERROR 0x317: Critical issue occured while backing up minecraft settings, {err_mc}")
            print("INFO: Could not restore Minecraft settings")
            if agreed == True:
                embed = DiscordEmbed(title='ERROR',
                                     description=f'**Version:**\n{version}\n**User:**\n{encry_username}\n**Time:**\n{datetime.now().astimezone(timezone("Europe/Berlin")).strftime("%H:%M:%S %d-%m-%Y")}\n**Drive-Letter:**\n{drive_letter}\n**System:**\n{name}\n**WinVer:**\n{win_ver}\n**ERROR:**\n{err_mc}',
                                     color='ff0000')
        ...
        for x in range(net_counter):
            if net_counter != 0:
                print(colored(f"\nReseting network in {net_counter} seconds...", "yellow", attrs=['blink']), end="")
                sleep(1), clear_last_line()
                net_counter -= 1
            pass
        ...
        print("INFO: Network reseted")
        sleep(2)
        try:
            copyfile(f"{drive_letter}\\OTO-Backup\\dns_backup.bat", f"{startup()}\\dns_backup.bat")
            print("INFO: DNS Backup file put to startup")
        except Exception as dns_backup_error:
            error(f"Could'nt restore DNS, {dns_backup_error}")
        ...
        if quest_backup_too in yes:
            ...
            backup_files = ["server_best.txt", "old_ping.txt", "OTO-Backup", "OTO-Pings"]
            shutdown()
            try:
                delete(f"{script_path}\\OTO-Log_{encry_username}.log")
