...
for link in links:
    ...
    try:
        ...
        if counter < 10:
            counter = str(counter)
            counter = "0" + counter
        print(f'({counter}/08) Downloaded {link.split("/")[-1]}')
        counter = int(counter)
        ...
        if counter < 10:
            ...
            counter = str(counter)
            counter = "0" + counter
        print(f'({str(counter)}/08){space}', colored(f'Error {link.split("/")[-1]}', 'red'))
...
try:
    ...
    if counter < 10:
        counter = str(counter)
        counter = "0" + counter
    print(f'({str(counter)}/08) Downloaded {unique_link.split("/")[-1]}')
except Exception as error____:
    counter += 1
    download_error = True
    error(f"ERROR 0x2: Critical issue occured while downloading file, {error____}")
    print(f'({str(counter)}/08)', colored(f'Error {unique_link.split("/")[-1]}', 'red'))
...
try:
    ...
    error(f"Error occured while connection to speedtest!, {connect_err}")
    show_menu()
    print(colored("Error occured while connection to speedtest server!", "red"))
    input("Press enter to exit...")
    exit()
...
if choice == "1":
    ...
    if question_3 in yes:
        ...
        debug("Called!")
        '''
        print("This function has been temporarily deactivated!")
        sleep(3)
        clear()
        show_menu()
        print("Done! Continuing...")
    ...
    question_5 = input("Do you want to unpark your cpu? (y/n)\nEnter: ")
    ...
    if question_6 in yes:
        ...
        os.chdir(gettempdir())
        system(r"%windir%\system32\reg.exe import test2.reg")
        debug("Called!")
        sleep(3)
        clear()
        show_menu()
    ...
    if question_7 in yes:
        quest_wifi_or_eth = input("  - Are you using Wi-Fi (wireless wlan) or Ethernet (PC connected with lan cable)? (w/e): ")
        if quest_wifi_or_eth == "w":
            connection_type = "Wi-Fi"
        elif quest_wifi_or_eth == "e":
            connection_type = "Ethernet"
        else:
            debug("Error/Issue: No valid input at line 794.")
            input("Invalid input, enter to exit...")
            exit()
    ...
    if yeono == True:
        ...
        print("You first need to optimize your PC before you can compare your old ping before optimization and new ping!")
        input("\nPress enter to exit...")
        exit()
elif choice == "3":
